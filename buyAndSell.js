...
async function sellPosition(
    ...
    try {
        const priceToSell = (
            currentPrice -
            currentPrice * tradingConfig.orderPriceDelta
        ).toFixed(productInfo.quoteIncrementRoundValue);
        ...
        for (let i = 0; i < 100 && positionInfo.positionExists === true; ++i) {
            ...
            try {
                ...
                const message =
                    "Error occured when attempting to get the order.";
            ...
            if (orderDetails.status === "done") {
                if (orderDetails.done_reason !== "filled") {
                    throw new Error(
                        "Sell order did not complete due to being filled? done_reason: " +
                            orderDetails.done_reason
                    );
                } else {
                    ...
                    try {
                        ...
                        fileSystem.writeFileSync(
                            "data/positionData.json",
                            writeData
                        );
                    } catch (err) {
                        const message =
                            "Error, failed to write the positionInfo to the positionData file in sellPosition. Continuing as normal but but positionDataTracking might not work correctly.";
                        const errorMsg = new Error(err);
                        logger.error({ message, errorMsg, err });
                    }

                    let profit =
                        parseFloat(orderDetails.executed_value) -
                        parseFloat(orderDetails.fill_fees) -
                        positionInfo.positionAcquiredCost;
                    ...
                    if (profit > 0) {
                        ...
                        if (depositConfig.depositingEnabled) {
                            const transferAmount = (
                                profit * depositConfig.depositingAmount
                            ).toFixed(2);
                            const currency = productInfo.quoteCurrency;

                            //Transfer funds to depositProfileID
                            const transferResult =
                                await coinbaseLibObject.profileTransfer(
                                    accountIds.tradeProfileID,
                                    accountIds.depositProfileID,
                                    currency,
                                    transferAmount
                                );
                        ...
                        throw new Error(
                            "Sell was not profitable, terminating program. profit: " +
                                profit
                        );
                    }
                }
        ...
        if (positionInfo.positionExists === true) {
            ...
            if (cancelOrder !== orderID) {
                throw new Error(
                    "Attempted to cancel failed order but it did not work. cancelOrderReturn: " +
                        cancelOrder +
                        "orderID: " +
                        orderID
                );
...
async function buyPosition(
    ...
    try {
        ...
        if (productInfo.baseIncrementRoundValue === 0) {
            ...
            orderSize = (amountToSpend / priceToBuy).toFixed(
                productInfo.baseIncrementRoundValue
            );
        }
        ...
        for (let i = 0; i < 100 && positionInfo.positionExists === false; ++i) {
            ...
            if (orderDetails.status === "done") {
                if (orderDetails.done_reason !== "filled") {
                    throw new Error(
                        "Buy order did not complete due to being filled? done_reason: " +
                            orderDetails.done_reason
                    );
                } else {
                    //Update position info
                    positionInfo.positionExists = true;
                    positionInfo.positionAcquiredPrice =
                        parseFloat(orderDetails.executed_value) /
                        parseFloat(orderDetails.filled_size);
                    positionInfo.positionAcquiredCost =
                        parseFloat(orderDetails.executed_value) +
                        parseFloat(orderDetails.fill_fees);
                    ...
                    try {
                        ...
                        const message =
                            "Error, failed to write the positionInfo to the positionData file in buyPosition. Continuing as normal but but positionDataTracking might not work correctly.";
                        const errorMsg = new Error(err);
                        logger.error({ message, errorMsg, err });
                    }
